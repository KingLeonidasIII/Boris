...
function clearObjects(){
 bullets.length=enemies.length=orbList.length=particles.length=hazards.length=texts.length=0
}
...
function updateArmory(){
 const w=weapon();
 const path=weaponPath?weaponUpgrades[w.id].find(u=>u.id===weaponPath):null;

 let html=`
 <h1 style="color:${w.color}">ARMORY</h1>
 <p>Orbs: <strong style="color:${C.yellow}">${orbs}</strong> · Wave ${wave}</p>
 ...
 <h2>${w.name} PATHS ${path?`— ${path.name.split(" PATH")[0]}`:""}</h2>
 <div class="grid">`;
 ...
 <p class="small">P / ESC closes Armory. The next wave begins after you close it.</p>`;
...
function waveBudget(n){
 return Math.floor(5+n*2.3+Math.pow(n,1.18)*.8);
}
...
function waveReward(){
 return 2+Math.floor(wave*.75)+(waveModifier.id==="blood"?2:0);
}
...
function spawnElite(){
 const types=["fast","tank","charger","shooter","leech"];
 return spawnEnemy(types[(wave*3)%types.length],true);
}
...
function edge(m=50){
 ...
 if(s===1)return{x:W+m,y:Math.random()*H};
 if(s===2)return{x:Math.random()*W,y:H+m};
...
function shoot(){
 ...
 if(w.id==="scatter"){
  ...
  const spread=w.spread*(1+.18*lvl("scatterWide"));
  ...
  for(let i=0;i<n;i++){
   ...
   makeBullet(
    a,
    player.damage*(.5+.10*lvl("scatterBuckshot")),
    {
  ...
  if(w.id==="missile")
   ex.explosion=w.explosion+18*lvl("missileWarhead");

  if(w.id==="arc"){
   ex.chain=w.chain+lvl("arcChain");
   ex.chainRange=w.chainRange+25*lvl("arcReach");
  }

  makeBullet(player.aim,player.damage,ex);

  if(w.id==="pulse"&&lvl("pulseEcho")&&Math.random()<.12*lvl("pulseEcho")){
   makeBullet(
    player.aim+(.04+Math.random()*.04)*(Math.random()<.5?-1:1),
    player.damage*.45,
    {
...
function hitPlayer(){
 if(has("shield")&&!player.emergencyShield){
  ...
  burst(player.x,player.y,C.blue,30,260);
  shake=7;
  return;
 }
...
function killEnemy(i){
 ...
 if(e.type!=="boss"&&Math.random()<.055&&orbList.length<4)
  spawnOrb();

 if(has("scavenger")&&Math.random()<.10&&orbList.length<4)
  spawnOrb();
 ...
 if(e.type==="boss")
  text(e.x,e.y-50,"BOSS DEFEATED",C.yellow);
...
function explode(x,y,r,damage,source){
 ...
 for(let i=enemies.length-1;i>=0;i--){
  ...
  if(e.type==="boss"&&source?.hit?.has(e))continue;
...
function chainAttack(x,y,damage,jumps,range,exclude){
 ...
 for(let n=0;n<jumps;n++){
  ...
  chainNodes.push({x:best.x,y:best.y});
...
function updateBullets(dt){
 for(let i=bullets.length-1;i>=0;i--){
  ...
  for(let j=enemies.length-1;j>=0&&!remove;j--){
   ...
   if(selectedWeapon==="rail"&&lvl("railShock")){
    hazards.push({
     x:e.x,
     y:e.y,
     r:18,
...
function updateEnemies(dt){
 for(let i=enemies.length-1;i>=0;i--){
  ...
  if(e.type==="charger"){
   ...
   if(e.charge<=0){
    e.chargeTime=.48;
    e.charge=e.chargeMax+Math.random();
    burst(e.x,e.y,C.orange,10,100);
   }

   if(e.chargeTime>0){
    e.chargeTime-=dt;
    e.x+=Math.cos(a)*e.speed*3.3*dt;
    e.y+=Math.sin(a)*e.speed*3.3*dt;
   }else{
   ...
   if(d>260){
    ...
    e.x-=Math.cos(a)*e.speed*.7*dt;
    e.y-=Math.sin(a)*e.speed*.7*dt;
   }
  ...
  if(e.type==="leech"){
   ...
   if(e.orbTimer<=0){
    const o=orbList.slice().sort((a,b)=>dist(e,a)-dist(e,b))[0];

    if(o&&dist(e,o)<260){
     o.x=e.x;
     o.y=e.y;
     burst(o.x,o.y,C.red,5,80);
    }
...
function updateBoss(e,dt){
 ...
 if(e.shot<=0){
  for(let i=0;i<5;i++)
   enemyShot({x:e.x,y:e.y});

  e.shot=2.2;
  burst(e.x,e.y,C.purple,18,150);
 }

 if(e.spawn<=0){
  for(let i=0;i<Math.min(2,Math.floor(wave/4));i++)
   spawnEnemy("fast");

  e.spawn=5;
 }

 if(e.hp<e.maxHp*.5&&e.phase>8){
  e.phase=0;
...
function updateHazards(dt){
 for(let i=hazards.length-1;i>=0;i--){
  ...
  if(h.type==="mine"&&h.life>0&&dist(h,player)<h.r){
   h.life=0;
   explode(h.x,h.y,58,1,{hit:new Set()});
   hitPlayer();
  }

  if(h.type==="warning"&&h.life<.35&&dist(h,player)<h.r){
   h.life=0;
   hitPlayer();
  }
...
function updateOrbs(dt){
 for(let i=orbList.length-1;i>=0;i--){
  ...
  if(has("magnet")){
   ...
   if(d>0&&d<85){
    o.x+=dx/d*180*(1-d/85)*dt;
    o.y+=dy/d*180*(1-d/85)*dt;
   }
  }
...
function updateWave(dt){
 ...
 if(waveEnemiesLeft>0){
  ...
  if(waveSpawnTimer<=0){
   spawnEnemy();
   waveSpawnTimer=Math.max(.22,.75-wave*.012);

   if(wave===1&&has("quickstart"))
    waveSpawnTimer*=1.2;
  }
 }
...
function drawPlayer(){
 ...
 if(player.dashCooldown<=0&&!gameOver){
  ...
  ctx.arc(
   player.x,
   player.y,
   player.r+10+Math.sin(performance.now()/100)*2,
   0,
   Math.PI*2
  );
  ctx.stroke();
...
function drawEnemies(){
 for(const e of enemies){
  ...
  ctx.shadowBlur=20;
  ctx.shadowColor=e.type==="boss"?C.purple:C.red;

  let col=
   e.flash>0?C.white:
   e.type==="boss"?C.purple:
   e.type==="fast"?C.orange:
   e.type==="leech"?C.yellow:
   e.type==="charger"?C.orange:
   e.type==="shooter"?C.blue:
   C.red;
  ...
  if(e.type==="boss"){
   ...
   ctx.fillRect(-e.r*.7,-e.r*.7,e.r*1.4,e.r*1.4);
   ctx.fillStyle=C.white;
...
function drawHazards(){
 for(const h of hazards){
  if(h.type==="explosion"){
   ...
   ctx.beginPath();
   ctx.arc(h.x,h.y,h.maxR*p,0,Math.PI*2);
   ctx.stroke();
  }else if(h.type==="warning"){
   ctx.strokeStyle=C.red;
   ctx.lineWidth=3;
   ctx.globalAlpha=.5+.5*Math.sin(h.pulse*12);

   ctx.beginPath();
   ctx.arc(h.x,h.y,h.r,0,Math.PI*2);
   ctx.stroke();
  }else if(h.type==="mine"){
   ctx.strokeStyle="rgba(255,23,68,.45)";
   ctx.lineWidth=2;
...
function drawUI(){
 ...
 ctx.font="14px Arial";
 ctx.fillText(
  "WAVE "+wave+" · "+waveState.toUpperCase(),
  20,
  52
 );
 ...
 if(waveModifier&&waveState==="combat"){
  ctx.fillStyle=C.orange;
  ctx.fillText(waveModifier.name,W-20,52);
...
function updateMessage(){
 ...
 if(homeScreen){
  message.innerHTML=`
   ...
   <p>Survive escalating waves, spend limited resources between waves, and build a specialized weapon.</p>
   ...
   <p class="small">ENTER / SPACE — Play · B — Abilities · V — Weapons</p>`;
 ...
 if(waveState==="intermission"){
  message.innerHTML=`
   <h1 style="color:${C.cyan}">WAVE ${wave} COMPLETE</h1>
   <p>Breathing room: <strong>${Math.ceil(intermission)}s</strong></p>
   <p>You received <strong style="color:${C.yellow}">${waveReward()} orbs</strong>. No enemies or orbs spawn during the break.</p>

   <button class="main-button" onclick="openArmory()">OPEN ARMORY</button>
   <button class="main-button secondary-button" onclick="closeArmory()">SKIP</button>

   <p class="small">P — Armory during the break</p>`;
...
function update(dt){
 ...
 timeAlive+=dt;
 score=Math.max(score,Math.floor(timeAlive*4));
