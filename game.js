...
let W=0,H=0,dpr=1;
const C={
 bg:"#05050a",cyan:"#00ffff",red:"#ff1744",yellow:"#ffe600",white:"#fff",
 pink:"#ff66cc",orange:"#ff9d00",purple:"#a66cff",green:"#66ffcc",blue:"#66ccff"
};
...
let score=0,orbs=0,timeAlive=0,wave=0,waveState="home",
 waveEnemiesLeft=0,waveSpawnTimer=0;
...
const player={
 ...
 damage:1,bulletSpeed:850,
 dashCooldown:0,dashMax:1.1,
 dashTime:0,dashDuration:.12,dashSpeed:1000,
 dashX:1,dashY:0,
...
const WEAPONS=[
 ...
 },
 {
  ...
  rate:.58,damage:4,speed:1400,life:1.2,pierce:2
 },
 {
  id:"scatter",
  name:"BLAST SHOTGUN",
  ...
  rate:.34,damage:.52,speed:800,life:.85,pellets:7,spread:.72
 },
 {
  id:"laser",
  name:"LASER",
  ...
  rate:.14,damage:.72,speed:1900,life:.32,pierce:4,laser:true
 },
 {
  id:"missile",
  ...
  rate:.52,damage:3,speed:500,life:2.2,explosion:62
 },
 {
  id:"arc",
  name:"ARC CASTER",
  ...
  rate:.25,damage:1.2,speed:1000,life:1.1,chain:2,chainRange:125
 }
];
...
const ABILITIES=[
 ...
 {id:"magnet",name:"MAGNETIC CORE",desc:"Attract yellow orbs from 110 pixels away.",color:C.yellow},
 {id:"scavenger",name:"SCAVENGER",desc:"Kills have a 12% chance to create an extra orb.",color:C.yellow},
 {id:"quickstart",name:"QUICK START",desc:"Enemy spawning is 25% slower for the first five seconds of each run.",color:C.cyan},
 {id:"overcharge",name:"OVERCHARGED CORE",desc:"Weapon damage +15%.",color:C.orange},
 {id:"stabilizers",name:"STABILIZERS",desc:"Movement speed +10%.",color:"#00ff88"},
 {id:"afterburner",name:"AFTERBURNER",desc:"Dash cooldown -12%.",color:C.purple},
 {id:"quickhands",name:"QUICK HANDS",desc:"Weapon cooldown -10%.",color:C.cyan},
 {id:"barrel",name:"THICK BARREL",desc:"Projectile speed and lifetime +12%.",color:"#ff7b00"},
 {id:"collector",name:"COLLECTOR",desc:"Every fifth orb collected gives +1 additional orb.",color:C.yellow},
 ...
 {id:"phase",name:"PHASE DRIVE",desc:"You cannot be damaged while dashing.",color:C.blue},
 {id:"adrenaline",name:"ADRENALINE",desc:"At 1 HP, weapon cooldown is reduced by 15%.",color:C.red},
 {id:"velocity",name:"VELOCITY CORE",desc:"Projectile speed +15%.",color:C.green},
 {id:"executioner",name:"EXECUTIONER",desc:"Deal +20% damage to enemies below half health.",color:C.orange},
 {id:"fieldmedic",name:"FIELD MEDIC",desc:"Restore 1 HP after every third completed wave.",color:C.pink}
];
...
const general={
 thrusters:{
  ...
  cost:4,max:3,level:0
 },
 heavy:{
  name:"HEAVY WEAPON",
  desc:"+0.45 damage per level.",
  cost:5,max:2,level:0
 },
 overdrive:{
  name:"OVERDRIVE",
  desc:"Dash cooldown -10% per level.",
  cost:6,max:2,level:0
 }
};
...
function applyUpgrades(){
 ...
 if(has("adrenaline")&&player.health===1)rate*=.85;

 rate*=1+.04*general.heavy.level;

 if(selectedWeapon==="pulse")
  rate*=1+.08*lvl("pulseSurge")+.06*lvl("pulseEcho");

 if(selectedWeapon==="rail")
  rate*=1+.08*lvl("railAccelerator");

 if(selectedWeapon==="laser")
  rate*=1+.08*lvl("laserReach");

 if(selectedWeapon==="missile")
  rate*=1+.08*lvl("missileFuel")+.06*lvl("missilePayload");

 if(selectedWeapon==="arc")
  rate*=1+.06*lvl("arcVoltage");

 if(selectedWeapon==="scatter")
  rate*=1+.05*lvl("scatterBuckshot");

 player.fireRate=Math.max(.07,rate);

 let damage=w.damage+.45*general.heavy.level;
 ...
 player.moveSpeed=
  300*
  (has("stabilizers")?1.1:1)*
  (1+.06*general.thrusters.level);

 player.dashMax=
  1.1*
  (has("afterburner")?.88:1)*
  Math.pow(.9,general.overdrive.level);
...
function updateAbilityMessage(){
 message.innerHTML=`
  ...
  <p>
   Choose two permanent abilities. These are deliberately stronger,
   direct upgrades; build diversity comes from having only two slots.
  </p>
...
function updateWeaponMessage(){
 message.innerHTML=`
  ...
  <p>
   Choose one permanent weapon. Its Armory path is selected during the run.
  </p>

  <div class="grid">
   ${WEAPONS.map(x=>`
    <div class="card ${x.id===selectedWeapon?"selected":""}"
         onclick="selectWeapon('${x.id}')">
     ...
     <p>
      Base damage ${x.damage}
      · cooldown ${x.rate.toFixed(2)}s
     </p>
...
function openArmory(){
 if(gameOver||homeScreen||!waveState||waveState==="home")return;
...
function updateArmory(){
 ...
 let html=`
  ...
  <p class="small">
   You get limited build choices. Picking a weapon path locks
   the other paths for this run.
  </p>
 ...
 html+=`
  ...
  <button class="main-button secondary-button"
          onclick="closeArmory()">
   ${waveState==="intermission"?"CLOSE ARMORY":"RESUME ROUND"}
  </button>

  <p class="small">
   P / ESC closes Armory. The wave timer is paused while it is open.
  </p>
 `;
...
window.skipIntermission=skipIntermission;
...
function waveReward(){
 return wave>0
  ?2+Math.floor(wave*.75)+(waveModifier?.id==="blood"?2:0)
  :0;
}
...
function chooseEnemyType(){
 ...
 if(n>=2&&r<.14+(m==="hunters"?.12:0))
  return"fast";

 if(n>=3&&r>.72-(m==="fortified"?.12:0))
  return"tank";
...
function hitPlayer(){
 if(has("phase")&&player.dashTime>0)return;
...
function updateBullets(dt){
 for(let i=bullets.length-1;i>=0;i--){
  ...
  for(let j=enemies.length-1;j>=0&&!remove;j--){
   ...
   /*
    * Explosion/chain attacks can remove the current enemy.
    * Check the reference instead of blindly using j afterwards.
    */
   const stillExists=enemies.includes(e);
...
function updateParticles(dt){
 for(let i=particles.length-1;i>=0;i--){
  ...
  if(p.type==="chain"){
   // Chain geometry is static for its short lifetime.
  }else{
   p.x+=p.dx*dt;
   p.y+=p.dy*dt;
...
function completeWave(){
 ...
 /*
  * No persistent orbs survive into the calm period.
  * New ones are blocked by spawnOrb() while intermission is active.
  */
 orbList.length=0;
 hazards.length=0;
 ...
 /*
  * FIELD MEDIC is intentionally checked here rather than every frame.
  * Every third completed wave restores one HP.
  */
 if(
  has("fieldmedic")&&
  wave%3===0&&
...
function updateWave(dt){
 ...
 /*
  * Every fifth wave becomes a boss wave.
  * The normal wave budget must be exhausted first.
  */
 if(
  wave%5===0&&
  waveEnemiesLeft===0&&
 ...
 /*
  * The wave cannot finish merely because its spawn budget
  * reached zero. All currently active enemies must also die.
  */
 if(
  waveEnemiesLeft===0&&
  enemies.length===0
...
function drawBullets(){
 for(const b of bullets){
   ...
   * LASER
   *
   * The trail is deliberately rendered behind the projectile,
   * while the main beam remains bright and compact.
   */
  if(
   b.laser&&
   b.trail.length>1
  ...
  if(b.laser){
    ...
    * MISSILE
    *
    * The projectile itself is compact; the explosion is rendered
    * separately through drawHazards().
    */
   ctx.shadowBlur=20;
  ...
  }else if(weapon().id==="arc"){

   /*
    * ARC CASTER
    *
    * No long horizontal projectile bar. The projectile is a
    * compact electrical orb with a small zig-zag core.
    */
   ctx.shadowBlur=20;
...
function drawHazards(){
 for(const h of hazards){
  ...
  if(h.type==="explosion"){
   ...
   /*
    * Additional explosion core.
    * This makes missile impacts clearly visible even on small screens.
    */
   if(p<.65){
    ctx.globalAlpha=(.65-p)*1.4;
    ctx.fillStyle=C.purple;
...
function updateMessage(){
 ...
 if(homeScreen){
  message.innerHTML=`
   ...
   <p>
    Survive escalating waves, spend limited resources
    between waves, and build a specialized weapon.
   </p>
 ...
 if(waveState==="intermission"){
  message.innerHTML=`
   ...
   <p class="small">
    P — Open Armory · SKIP — Start next wave immediately
   </p>
  `;
